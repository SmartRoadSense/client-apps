...
namespace SmartRoadSense.Shared.Data {
    ...
    public abstract class SensorPack {
        ...
        public static SensorPack Create(Engine engine) {
            ...
            return new SensorPackWinRT(engine);
#elif DESKTOP
        ...
        public LocationSensorStatus LocationSensorStatus {
            ...
            protected set {
                ...
                if (_locationSensorStatus != value) {
                    Log.Debug("Sensor status {0} -> {1}", _locationSensorStatus, value);
...
#region Common sensor data handling
        ...
        private float _gpsLastBearing = 0f;
        private int _gpsLastAccuracy = 0;
        ...
        private void FeedEngine(DataEntry data) {
            try {
                ...
                Log.Error(ex, "Internal engine computation error");
                _engine.Reset();

                OnInternalEngineErrorReported(ex);
            }
            catch (ArgumentException ex) {
                Log.Debug("Engine data error ({0})", ex.Message);
                _engine.Reset();
            }
            catch (Exception ex) {
                Log.Error(ex, "Unforeseen internal engine error");
                _engine.Reset();
            }
        }
        ...
        public void StartSensing() {
            if (_isActive) {
                Log.Debug("Sensors already running");
                return;
            }
            ...
            try {
                ...
                Log.Error(ex, "Failed started sensing");
                _isActive = false;
                return;
            }

            Log.Debug("Sensor pack started sensing");
            _isActive = true;
        }
        ...
        public void StopSensing() {
            if (!_isActive) {
                Log.Debug("Sensors already stopped");
                return;
            }

            StopSensingCore();

            Log.Debug("Sensor pack stopped sensing");
            _isActive = false;
        }
